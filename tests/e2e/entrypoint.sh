...
# Install the local tarball to node user's home (for CI/CD testing before publishing)
npm i --prefix /home/node /packages/n8n-observability/*.tgz
...
# Try to find the workflow by name "Simple Test Workflow"
...
WORKFLOW_FILE=$(grep -l "Simple Test Workflow" /tmp/exports/*.json 2>/dev/null | head -n1 || true)
